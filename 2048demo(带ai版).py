...
def phase(board):#具体分为5个阶段，最后一阶段为处理移动的阶段
    ...
    if (board.phase == 0):
        ...
        if (board.option in [1,3]):
            pos = player.play(board.ln,board.col,board.numbers,board.ownership,0,0)
            posclick(board,pos[0],pos[1]).proc()
    elif (board.phase == 1):
        board.label["text"] = "B摆放阶段"
        if (board.option in [2,3]):
            pos = player.play(board.ln,board.col,board.numbers,board.ownership,1,0)
            posclick(board,pos[0],pos[1]).proc()
    elif (board.phase == 2):
        board.label["text"] = "A指定方向阶段"
        if (board.option in [1,3]):
            direction = player.play(board.ln,board.col,board.numbers,board.ownership,1,1)
            dirselect(board,"A",direction).proc()
    elif (board.phase == 3):
        board.label["text"] = "B指定方向阶段"
        if (board.option in [2,3]):
            direction = player.play(board.ln,board.col,board.numbers,board.ownership,1,1)
            dirselect(board,"B",direction).proc()
    else:
        proc_move(board)
        phase(board)
...
class Board():#对于棋盘的处理
    def __init__(self, root, ln, col):
        def board_init(self,root, ln, col):
            ...
            self.label = Label(root, width=30, height=2, bg='white', anchor='se', text="请选择模式")
            ...
            phase(self)
        def option(self, root, ln, col):
            def option0():
                self.option=0
                root1.destroy()
                board_init(self, root, ln, col)
            def option1():
                self.option=1
                root1.destroy()
                board_init(self, root, ln, col)
            ...
            def option3():
                ...
                self.option=3
                root1.destroy()
                Button(root,text="继续",width=5,command=nextphase).grid(row=ln+4,column=1)
                board_init(self, root, ln, col)
            root1=tkinter.Tk()
            self.label = Label(root1, width=30, height=2, bg='white', anchor='se', text="请选择模式")
            self.label.grid(row=0, columnspan=8)
            Button(root1,text="左右互搏",width=5,command=option0).grid(row=0,column=0,columnspan=2)
            Button(root1,text="先手随机",width=5,command=option1).grid(row=0,column=2,columnspan=2)
            Button(root1,text="后手随机",width=5,command=option2).grid(row=0,column=4,columnspan=2)
            Button(root1,text="全部随机",width=5,command=option3).grid(row=0,column=6,columnspan=2)
        option(self, root, ln, col)
