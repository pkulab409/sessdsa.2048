...
方块合并：游戏操作，需要一个方向作为参数（上、下、左、右）。本方全部棋子向该方向运动，直到以下两种情况：1、即将越出可并范围，停下；2、遇到对方棋子或已经停下的本方棋子，如果这两个棋子数值相等，运动的棋子将“吃掉”停下的棋子，占据停下的棋子原先的位置，并将自身数值修改为原数值的两倍，停下（显然按照此规则，沿合并方向靠前的棋子将优先结算）。

**注记**：作为例子，设定合并方向向右：2、2、4 -> 4、4；4、2、2 -> 8；2、2、2、2 -> 4、4
